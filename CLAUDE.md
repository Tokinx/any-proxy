...
ALLOWLIST="10.0.0.0/24" bun proxy.js            # 启用白名单
WS_QUEUE_LIMIT_BYTES=2097152 bun proxy.js       # 调整 WS 建连前缓冲上限
```
...
1. 判定阶段：先做白名单检查；WebSocket 允许规则是「来源 IP 或下游目标 IP 任一命中白名单即可」
2. 同步握手阶段：调用 `srv.upgrade()`，把 `wsTarget`、客户端期望的子协议、要透传的 headers 全部存到 `ws.data`，并立即 echo 第一个 `sec-websocket-protocol` 给客户端
3. 异步建连阶段：`websocket.open` 钩子才真正向下游发起 `new WebSocket(...)`，连接成功前到达的客户端消息进 `ws.data.queue`，`downstream open` 触发后冲刷队列

修改这一段时务必保留以下不变量，否则会出现握手成功但消息丢失/卡死：

- 子协议在 upgrade 阶段就要选定一个返回给客户端，下游若不接受会主动 close（让客户端自己处理）
- 消息队列必须在 `downstream.open` 里**先冲刷再清空**，并受 `WS_QUEUE_LIMIT_BYTES` 保护，避免未建连时无限堆积
...
- HTTP/HTTPS 仅按来源 IP 校验
- WebSocket 只要来源 IP 或下游目标 IP 任一命中白名单，即允许连接
- 列表为空时全部放行
- 同时支持 `::ffff:1.2.3.4` 形式的 IPv4-mapped IPv6
- systemd 部署下白名单源是 `/etc/any-proxy.allowlist`，install.sh 在生成 service 文件时将其拼成 `ALLOWLIST=` 环境变量；Docker 部署直接传环境变量
...
|------|------|
| `PORT` | 监听端口，默认 3000 |
| `ALLOWLIST` | 逗号分隔的 IP/CIDR |
| `WS_QUEUE_LIMIT_BYTES` | WS 下游未就绪前的缓冲字节上限，默认 1048576 |

## 修改代码时的注意事项

- 没有 `package.json`，不要引入 npm 依赖——保持「拷一个文件就能跑」是该项目的核心卖点
- 已有 Bun 测试；功能改动至少运行 `bun test`
